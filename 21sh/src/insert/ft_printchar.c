...
/*   Updated: 2018/04/21 15:01:22 by tcharrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_printchar(t_line *line, char *str, int *val)
{
	...
	if ((val[10] = val[0] + val[9] + ft_strlen(&tmp[val[0]])) >=
		...
		if (ft_realloc_line(line, val, (val[4] ? line->size_line :
			line->size_eof) + 30) == -1)
			return (-1);
	tmp = (val[4] ? line->line : line->eof);
	...
	write(1, str, ft_lenchar_r(str, 0));
...
int		ft_printinsert(t_line *line, int *val)
{
	...
	if (com[val[0] + val[11] - 1] == '\n' && val[12] >= val[11] && loc[0] == val[7])
		val[11]--;
	write(1, &com[val[0]], val[11] + val[10]);
